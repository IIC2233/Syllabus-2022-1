...
print("*** Bienvenido a Restaurant D'Elliott ***")

dict_ingredientes = cargar_ingredientes("ingredientes.csv")
bodega = generar_bodega(dict_ingredientes)
dccRestaurant = Cocina(bodega, recetas=dict_ingredientes)
cocineros = [Cocinero(x, dccRestaurant) for x in ('Caua Paz', 'Cami Gonzalez',
                                                  'Benja Aguila', 'Hanan Saab')]
meseros = [Mesero(x) for x in (f'Mesero {i}' for i in range(1, 10))]

dccRestaurant.meseros = meseros
dccRestaurant.cocineros = cocineros

dccRestaurant.initialize_threads()
...
for pedido in cargar_pedidos("pedidos.csv"):
